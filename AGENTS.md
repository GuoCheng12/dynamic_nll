...
- Directory layout is fixed: `configs/` (Hydra: `config.yaml`, `dataset/`, `experiment/`), `src/` (`loss.py`, `models.py`, `dataset.py`, `utils.py`, `__init__.py`), root `train.py`, `eval.py`, and `README.md`.
- Models in `models.py` must output both mean and log_variance. Keep datasets swappable via factories in `dataset.py`; utilities (metrics, grad norms) live in `utils.py`.
- Add diagrams or notes in `README.md` or `configs/experiment/` to document β schedules and dataset choices.
...
## Core Logic: Loss & Scheduler (src/loss.py)
...
- Ensure variance passed to the loss is positive (e.g., apply `softplus`/`exp` in `models.py`).
